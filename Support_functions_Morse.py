...
def Merger_2(X,psi,X_b,psi_b,e):
    
    psi_b=np.copy(psi_b[::-1]/max(psi_b))
    psi_m=np.copy(psi/max(psi))# the final merged psi
    V_x=Poten(X)

    active_range=np.where(V_x<=e)
    ind_lw=active_range[0][0]
    ind_up=active_range[0][-1]
    p1,A=Normalize(X[ind_lw:ind_up],psi[ind_lw:ind_up])
    p2,A_b=Normalize(X_b[ind_lw:ind_up],psi_b[ind_lw:ind_up]) 
    psi_m=psi_m/A
    psi_b=psi_b/A_b
    
    last_min=0#stores the postion of last minimum in the psi array this indicates the minimum before the explotion
    max_psi=abs(max(psi[ind_lw:ind_up]))
    min_psi=abs(min(psi[ind_lw:ind_up]))
    if min_psi>max_psi:
        max_psi=min_psi
    if max_psi<=abs(psi[-1]):#detecting an explosion near the far end
        for p in range(len(psi)-2):
